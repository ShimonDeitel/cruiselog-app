import XCTest
@testable import Cruiselog

@MainActor
final class CruiselogTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        ...
        let added = store.add(Cruise(title: "Test", value2: 0))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        for i in 0..<Store.freeLimit {
            _ = store.add(Cruise(title: "Item \(i)", value2: 0))
        }
        let result = store.add(Cruise(title: "Overflow", value2: 0))
    ...
    func testDeleteRemovesItem() {
        let item = Cruise(title: "ToDelete", value2: 0)
        _ = store.add(item)
    ...
    func testUpdateModifiesItem() {
        var item = Cruise(title: "Original", value2: 0)
        _ = store.add(item)
        item.title = "Updated"
        store.update(item)
    ...
    func testDeleteAtOffsets() {
        _ = store.add(Cruise(title: "A", value2: 0))
        _ = store.add(Cruise(title: "B", value2: 0))
    ...
    func testPersistenceRoundTrip() {
        _ = store.add(Cruise(title: "Persisted", value2: 0))
        store.save()
        let reloaded = Store()
